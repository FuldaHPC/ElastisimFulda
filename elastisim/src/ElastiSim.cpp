...
void ElastiSim::startSimulation(int argc, char* argv[]) {
	...
	// Parse command-line arguments for native scheduler selection BEFORE loading config
	std::string nativeSchedulerName = "rigid_fifo";  // default
	bool schedulingPointFastPathOverride = false;
	bool schedulingPointFastPathEnabled = false;
	for (int i = 1; i < argc; i++) {
		std::string arg(argv[i]);
		if (arg == "--native-scheduler" && i + 1 < argc) {
			nativeSchedulerName = argv[++i];
			std::cout << "Using native scheduler: " << nativeSchedulerName << std::endl;
		} else if (arg == "--enable-scheduling-point-fast-path") {
			schedulingPointFastPathOverride = true;
			schedulingPointFastPathEnabled = true;
			std::cout << "Enabling scheduling-point fast path" << std::endl;
		} else if (arg == "--disable-scheduling-point-fast-path") {
			schedulingPointFastPathOverride = true;
			schedulingPointFastPathEnabled = false;
			std::cout << "Disabling scheduling-point fast path" << std::endl;
		}
	}

	Configuration::init(argv[1]);

	// Set native scheduler name in configuration
	Configuration::set("native_scheduler_name", nativeSchedulerName);
	if (schedulingPointFastPathOverride) {
		Configuration::set("enable_scheduling_point_fast_path", schedulingPointFastPathEnabled);
	}
	PerformanceTracer::initialize();
	...
	// Initialize shutdown policy if configured
	if (Configuration::exists("shutdown_policy_file")) {
		std::string policyFile = Configuration::get("shutdown_policy_file");
		ShutdownPolicyManager::init(policyFile);
	}
