...
void Sensing::operator()() const {
	...
	size_t numberOfLinks = 0;
	double pfsReadBandwidth = 0;
	double pfsWriteBandwidth = 0;
	{
		PerformanceTracer::ScopedComponent component("sensing_setup");
		...
		pfsReadBandwidth = PlatformManager::getPfsReadBandwidth();
		pfsWriteBandwidth = PlatformManager::getPfsWriteBandwidth();
	...
	while (simgrid::s4u::this_actor::get_host()->is_on()) {
		...
		PerformanceTracer::setObservedSimTime(time);

		{
			PerformanceTracer::ScopedComponent component("sensing_iteration");
			cpuUtilization << time << ",";
			gpuUtilization << time << ",";
