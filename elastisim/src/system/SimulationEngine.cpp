...
void SimulationEngine::operator()() {
	...
	while (expectedJobs > 0) {
		...
		{
			PerformanceTracer::ScopedComponent component("simulation_engine_handle_message");
	...
	{
		PerformanceTracer::ScopedComponent component("simulation_engine_finalize_stats");
		...
		for (const auto& job: jobs) {
			...
			} else if (job->getState() == RUNNING) {
				jobStatistics << "unfinished" << std::endl;
			} else {
				// PENDING, PENDING_ALLOCATION, PENDING_SUBMISSION, etc.
				// Can happen with shutdown policies when jobs can't be scheduled
				jobStatistics << "unscheduled" << std::endl;
			}
		}
	}
