...
#include "../scheduling/utils/NodeCache.h"
...
void Node::collectStatistics() {
	PerformanceTracer::ScopedComponent component("node_collect_statistics");
	std::string stateStr;
	switch (state) {
		case NODE_FREE:
			stateStr = "free";
	...
	PerformanceTracer::recordNodeUtilizationRow();
}

void Node::allocateJob(Job* job, int rank, const simgrid::s4u::BarrierPtr& jobBarrier) {
	PerformanceTracer::ScopedComponent component("node_allocate_job");
	...
	if (!runningJobs.empty()) {
		...
		NodeCache::invalidate();  // Cache invalidation: FREE → ALLOCATED
	}
	PlatformManager::addModifiedComputeNode(this);
	collectStatistics();
...
void Node::continueJob(Job* job) {
	PerformanceTracer::ScopedComponent component("node_continue_job");
	application[job] = s4u_Actor::create("Application@Job" + std::to_string(job->getId()), host,
										 Application(this, job, assignedRank[job], logTaskTimes));
}

void Node::reconfigureJob(Job* job, int rank, const simgrid::s4u::BarrierPtr& jobBarrier) {
	PerformanceTracer::ScopedComponent component("node_reconfigure_job");
	assignedRank[job] = rank;
	barrier[job] = jobBarrier;
...
void Node::expandJob(Job* job, int rank, int expandRank,
	...
	PerformanceTracer::ScopedComponent component("node_expand_job");
	assignedRank[job] = rank;
	...
	if (!runningJobs.empty()) {
		state = NODE_ALLOCATED;
		NodeCache::invalidate();  // Cache invalidation: expandJob FREE → ALLOCATED
	}
	PlatformManager::addModifiedComputeNode(this);
	collectStatistics();
	application[job] = s4u_Actor::create("Application@Job" + std::to_string(job->getId()), host,
...
void Node::completeJob(Job* job) {
	PerformanceTracer::ScopedComponent component("node_complete_job");
	application.erase(job);
	runningJobs.erase(job);
	if (runningJobs.empty()) {
		state = NODE_FREE;
		NodeCache::invalidate();  // Cache invalidation: ALLOCATED → FREE
	}
	PlatformManager::addModifiedComputeNode(this);
	collectStatistics();
}
...
void Node::killJob(Job* job) {
	PerformanceTracer::ScopedComponent component("node_kill_job");
...
void Node::expectJob(Job* job) {
	PerformanceTracer::ScopedComponent component("node_expect_job");
	...
	if (state == NODE_FREE) {
		state = NODE_RESERVED;
		NodeCache::invalidate();  // Cache invalidation: FREE → RESERVED
	}
	PlatformManager::addModifiedComputeNode(this);
}

void Node::removeExpectedJob(Job* job) {
	PerformanceTracer::ScopedComponent component("node_remove_expected_job");
	expectedJobs.erase(job);
	if (expectedJobs.empty() && state == NODE_RESERVED) {
		state = NODE_FREE;
		NodeCache::invalidate();  // Cache invalidation: RESERVED → FREE
	}
	PlatformManager::addModifiedComputeNode(this);
}
...
nlohmann::json Node::toJson() {
	PerformanceTracer::recordNodeToJsonCall();
	nlohmann::json json;
	json["id"] = id;
	json["type"] = type;
	json["state"] = state;
...
// Shutdown policy methods
NodeState Node::getState() const {
	return state;
}

void Node::setState(NodeState newState) {
	PerformanceTracer::ScopedComponent component("node_set_state");
	NodeCache::invalidate();  // Cache invalidation: shutdown policy transitions
	state = newState;
	collectStatistics();
	PlatformManager::addModifiedComputeNode(this);
}
...
bool Node::isIdle() const {
	return runningJobs.empty() && expectedJobs.empty();
}

void Node::scheduleTransition(NodeState target, int delaySeconds) {
	targetState = target;
	stateTransitionTime = simgrid::s4u::Engine::get_clock() + delaySeconds;
}

void Node::checkScheduledTransition(double currentTime) {
	if (stateTransitionTime > 0 && currentTime >= stateTransitionTime) {
		setState(targetState);
		stateTransitionTime = -1.0;
	}
}
