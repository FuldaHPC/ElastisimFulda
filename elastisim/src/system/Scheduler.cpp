...
#include "scheduling/base/IScheduler.h"
#include "scheduling/base/SchedulerRegistry.h"
#include "scheduling/base/SchedulingContext.h"
...
Scheduler::Scheduler(s4u_Host* masterHost) :
	...
	if (Configuration::exists("use_native_scheduler") && !Configuration::getBoolIfExists("use_native_scheduler")) {
		xbt_die("External scheduler support was removed from this native build. Set use_native_scheduler to true or omit it.");
	}

	std::string schedulerName = "rigid_fifo";
	if (Configuration::exists("native_scheduler_name")) {
		schedulerName = Configuration::get("native_scheduler_name");
	}

	try {
		nativeScheduler = SchedulerRegistry::create(schedulerName);
		XBT_INFO("Using native scheduler: %s", nativeScheduler->getName().c_str());
	} catch (const std::exception& e) {
		XBT_ERROR("Failed to create native scheduler '%s': %s", schedulerName.c_str(), e.what());
		throw;
	}
}
...
void Scheduler::schedule(InvocationType invocationType, Job* requestingJob, int numberOfNodes) {
	...
	if (minSchedulingInterval == 0 || clock - lastInvocation >= minSchedulingInterval - EPSILON) {
		PerformanceTracer::ScopedComponent scheduleComponent("scheduler_schedule");
		size_t pendingJobs = 0;
		size_t runningJobs = 0;
		size_t runningMalleableJobs = 0;
		for (Job* job : jobQueue) {
			...
			if (job->getState() == RUNNING) {
				...
				if (job->getType() == MALLEABLE) {
					runningMalleableJobs += 1;
				}
			}
		}
			PerformanceTracer::beginInvocation(nativeScheduler ? nativeScheduler->getName() : "unknown",
												invocationType,
												clock,
											jobQueue.size(),
											pendingJobs,
											...
											runningMalleableJobs,
												requestingJob ? requestingJob->getId() : -1,
												numberOfNodes);

			{
				PerformanceTracer::ScopedPhase phase("prediction_progress_update");
				JobUtils::updateRunningJobsPrediction(jobQueue, clock);
			}

			// Update shutdown policy
			const std::vector<Node*>& allNodes = PlatformManager::getComputeNodes();
		{
			PerformanceTracer::ScopedPhase phase("shutdown_update");
			ShutdownPolicyManager::update(clock, allNodes);
		}
		{
			PerformanceTracer::ScopedPhase phase("shutdown_malleable_policy");
			ShutdownPolicyManager::applyMalleableShutdownPolicy(jobQueue, allNodes, clock);
		}
		
		// Check scheduled state transitions for all nodes
		{
			PerformanceTracer::ScopedPhase phase("node_transition_check");
			for (Node* node : allNodes) {
				node->checkScheduledTransition(clock);
			}
		}
		...
		{
			PerformanceTracer::ScopedPhase phase("native_scheduler");
			scheduledJobs = nativeScheduler->schedule(ctx);
		}
		...
		if (invocationType == INVOKE_SCHEDULING_POINT || invocationType == INVOKE_EVOLVING_REQUEST) {
			if (requestingJob->getState() == PENDING_KILL) {
				PerformanceTracer::ScopedPhase phase("forward_kill");
				forwardJobKill(requestingJob, false);
			} else if (requestingJob->getState() == PENDING_RECONFIGURATION) {
				PerformanceTracer::ScopedPhase phase("forward_reconfiguration");
		...
		for (const auto& job: scheduledJobs) {
			if (job->getState() == PENDING_ALLOCATION) {
				PerformanceTracer::ScopedPhase phase("forward_allocation");
		...
		PerformanceTracer::endInvocation(scheduledJobs.size());
	}
}
...
void Scheduler::handleProcessedWorkload(Job* job) {
	PerformanceTracer::ScopedComponent component("scheduler_handle_processed_workload");
...
void Scheduler::forwardJobKill(Job* job, bool exceededWalltime) {
	PerformanceTracer::ScopedComponent component("scheduler_forward_kill");
...
void Scheduler::forwardJobAllocation(Job* job) {
	PerformanceTracer::ScopedComponent component("scheduler_forward_allocation");
	int rank = 0;
...
void Scheduler::handleReconfiguration(Job* job) {
	PerformanceTracer::ScopedComponent component("scheduler_handle_reconfiguration");
...
void Scheduler::operator()() {
	...
	while (true) {
		...
		{
			PerformanceTracer::ScopedComponent component("scheduler_handle_message");
