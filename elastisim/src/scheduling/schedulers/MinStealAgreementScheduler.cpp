...
XBT_LOG_NEW_DEFAULT_CATEGORY(MinStealAgreementScheduler, "MinStealAgreement scheduler");

MinStealAgreementScheduler::MinStealAgreementScheduler()
    : agreements(std::make_unique<StealAgreementHandler>()) {
    XBT_INFO("MinStealAgreementScheduler created!");
}

double MinStealAgreementScheduler::getExpandPriority(Job* job) {
    if (!job) {
        return 0.0;
    }
...
double MinStealAgreementScheduler::getShrinkPriority(Job* job) {
    return getExpandPriority(job);
}

std::vector<Job*> MinStealAgreementScheduler::schedule(const SchedulingContext& context) {
    context.validate();
        ...
        for (size_t i = 0; i < pendingNoAgree.size();) {
            ...
            if (eligibleNodes.size() >= static_cast<size_t>(reqNodes)) {
                ...
                bool delayed = false;
                {
                    ...
                    delayed = delaysHead(job, reqNodes, pendingNoAgree.front(), freeForHead);
                }
                if (delayed) {
                    ++i;
                    continue;
